    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_COMMANDER:
            ...
            if (!gSpecialStatuses[battler].switchInAbilityDone
             ...
             && gBattleMons[partner].species == SPECIES_MANTINE
             && GET_BASE_SPECIES_ID(GetMonData(GetPartyBattlerData(battler), MON_DATA_SPECIES)) == SPECIES_REMORAID)
            {
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case HOLD_EFFECT_HONEY:
        if (GetBattlerAbility(gBattleMons[battlerAtk].ability) == ABILITY_HONEY_GATHER)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    }
...
u32 GetBattlerMoveTargetType(u32 battler, u32 move)
{
    ...
    if (effect == EFFECT_TERA_STARSTORM && gBattleMons[battler].species == SPECIES_LEDIAN)
        return MOVE_TARGET_BOTH;
    if (effect == EFFECT_TERA_STARSTORM && gBattleMons[battler].species == SPECIES_JIRACHI)
        return MOVE_TARGET_BOTH;
    if (effect == EFFECT_TERA_STARSTORM && gBattleMons[battler].species == SPECIES_STARMIE)
        return MOVE_TARGET_BOTH;
